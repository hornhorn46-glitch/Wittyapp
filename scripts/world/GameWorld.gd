...
var game_over := false
var safe_zone_pos := Vector3(11.6, 0, -9.0)
...
func _build_environment() -> void:
	...
	env.ambient_light_color = Color(0.16, 0.18, 0.17)
...
func _create_material_zones() -> void:
	...
	_visual_box("CarpetRugStart", Vector3(0.3, 0.014, 1.55), Vector3(2.9, 0.018, 1.25), Color.WHITE)
	_visual_box("CarpetRugOffice", Vector3(6.5, 0.014, 1.8), Vector3(2.15, 0.018, 1.25), Color.WHITE)
	_visual_box("CarpetRugRescue", Vector3(13.1, 0.014, -7.65), Vector3(2.35, 0.018, 1.55), Color.WHITE)
...
func _create_door(position: Vector3, locked: bool = false, required_item: String = "") -> void:
	...
	var packed := load(FACTORY_MODELS + "door-wide-half.glb")
	if packed:
		var model: Node3D = packed.instantiate()
		model.scale = Vector3(1.2, 1.2, 1.2)
		model.rotation_degrees.y = 90
		model.position = Vector3(0, -0.02, 0)
		door.add_child(model)
	else:
		var mesh := MeshInstance3D.new()
		var box := BoxMesh.new()
		box.size = Vector3(0.18, 2.25, 1.25)
		mesh.mesh = box
		mesh.material_override = _material_for_node("DoorWood", Color(0.18, 0.12, 0.08))
		door.add_child(mesh)
	var collision := CollisionShape3D.new()
	var shape := BoxShape3D.new()
	shape.size = Vector3(0.18, 2.25, 1.25)
	collision.shape = shape
	door.add_child(collision)
	add_child(door)

func _create_cover(position: Vector3, size: Vector3) -> void:
	_box("Cover", position, size, Color(0.12, 0.11, 0.09))

func _create_props() -> void:
	for pos in [Vector3(-0.8, 0.45, -1.8), Vector3(2.0, 0.45, -1.7), Vector3(6.8, 0.38, -0.2), Vector3(13.9, 0.38, -6.2)]:
		_box("LowTable", pos, Vector3(1.1, 0.28, 0.55), Color(0.16, 0.10, 0.07))
	for pos in [Vector3(0.1, 1.1, -2.35), Vector3(7.0, 1.1, -2.05), Vector3(13.0, 1.1, -10.25)]:
		_box("WallSign", pos, Vector3(0.9, 0.42, 0.035), Color(0.04, 0.30, 0.27))
	...
	_add_model("screen-hanging-wide.glb", Vector3(13.1, 2.05, -3.7), Vector3(0, 180, 0), Vector3(0.9, 0.9, 0.9))
	...
	_add_model("button-floor-round.glb", Vector3(13.0, 0.03, -5.45), Vector3.ZERO, Vector3(0.42, 0.42, 0.42))
	...
	_add_model("structure-doorway-wide.glb", Vector3(3.05, 0.0, -0.9), Vector3(0, 90, 0), Vector3(1.15, 1.15, 1.15))
	_add_model("structure-doorway-wide.glb", Vector3(9.8, 0.0, -1.0), Vector3(0, 90, 0), Vector3(1.15, 1.15, 1.15))
...
func _create_furniture_set() -> void:
	...
	_add_furniture_model("chairRounded.glb", Vector3(1.65, 0.0, 1.85), Vector3(0, -45, 0), Vector3(0.62, 0.62, 0.62))
	...
	_add_furniture_model("chairDesk.glb", Vector3(6.55, 0.0, 1.58), Vector3.ZERO, Vector3(0.60, 0.60, 0.60))
	...
	_add_furniture_model("loungeSofaLong.glb", Vector3(12.55, 0.0, -9.38), Vector3.ZERO, Vector3(0.70, 0.70, 0.70))
	_add_furniture_model("sideTable.glb", Vector3(14.9, 0.0, -8.95), Vector3.ZERO, Vector3(0.55, 0.55, 0.55))
	_add_furniture_model("lampRoundTable.glb", Vector3(14.9, 0.55, -8.95), Vector3.ZERO, Vector3(0.42, 0.42, 0.42))
	_add_furniture_model("bookcaseClosedWide.glb", Vector3(10.85, 0.0, -8.9), Vector3(0, 90, 0), Vector3(0.64, 0.64, 0.64))
	_add_furniture_model("tableCoffee.glb", Vector3(13.35, 0.0, -7.25), Vector3.ZERO, Vector3(0.62, 0.62, 0.62))
	_add_furniture_model("loungeDesignSofa.glb", Vector3(14.85, 0.0, -6.25), Vector3(0, -90, 0), Vector3(0.60, 0.60, 0.60))
	...
	_add_furniture_model("rugRectangle.glb", Vector3(0.3, 0.018, 1.55), Vector3.ZERO, Vector3(0.82, 0.82, 0.82))
	_add_furniture_model("rugRounded.glb", Vector3(13.1, 0.018, -7.65), Vector3.ZERO, Vector3(0.72, 0.72, 0.72))
	_add_warm_lamp(Vector3(14.9, 1.05, -8.95), 1.1, 3.2)
	_add_warm_lamp(Vector3(11.0, 1.4, 1.35), 0.8, 3.0)

func _create_real_room_details() -> void:
	_box("ReceptionCounter", Vector3(1.25, 0.56, -1.95), Vector3(1.65, 1.12, 0.30), Color(0.18, 0.12, 0.075))
	_visual_box("ReceptionKickPlate", Vector3(1.25, 0.28, -1.785), Vector3(1.42, 0.18, 0.025), Color(0.04, 0.055, 0.052))
	_add_furniture_model("chairCushion.glb", Vector3(0.25, 0.0, -1.62), Vector3(0, 180, 0), Vector3(0.52, 0.52, 0.52))
	...
	for data in [
		...
		["chairModernCushion.glb", Vector3(15.15, 0.0, -8.55), Vector3(0, -125, 0), Vector3(0.56, 0.56, 0.56)],
	...
	_box("MaintenanceWorkBench", Vector3(18.4, 0.48, 1.0), Vector3(1.15, 0.96, 0.36), Color(0.13, 0.10, 0.075))
	_box("StorageShelfA", Vector3(16.25, 0.85, -2.8), Vector3(0.24, 1.7, 1.25), Color(0.06, 0.065, 0.06))
	_box("StorageShelfB", Vector3(16.25, 1.48, -2.8), Vector3(0.34, 0.08, 1.35), Color(0.08, 0.08, 0.075))
	_box("BreakRoomCounter", Vector3(11.05, 0.45, -6.2), Vector3(0.36, 0.9, 1.15), Color(0.16, 0.11, 0.08))
...
func _create_window(position: Vector3, size: Vector3, _rotation: Vector3) -> void:
	...
	if size.x > size.z:
		...
		_box("WindowFrame", position + Vector3(size.x * 0.5, 0, 0), Vector3(0.08, size.y + 0.08, size.z + 0.08), Color(0.02, 0.025, 0.027))
		_add_model("structure-window-wide.glb", position + Vector3(0, -0.52, 0.025 * sign(position.z)), Vector3(0, 0, 0), Vector3(0.52, 0.52, 0.52))
	else:
		_box("WindowFrame", position + Vector3(0, 0, -size.z * 0.5), Vector3(size.x + 0.08, size.y + 0.08, 0.08), Color(0.02, 0.025, 0.027))
		_box("WindowFrame", position + Vector3(0, 0, size.z * 0.5), Vector3(size.x + 0.08, size.y + 0.08, 0.08), Color(0.02, 0.025, 0.027))
		_add_model("structure-window.glb", position + Vector3(0.025 * sign(position.x), -0.52, 0), Vector3(0, 90, 0), Vector3(0.52, 0.52, 0.52))
...
func _create_architectural_details() -> void:
	...
	for pos in [Vector3(0, 0.22, -2.36), Vector3(6.5, 0.22, -2.06), Vector3(13, 0.22, 1.92), Vector3(13, 0.22, -10.2), Vector3(6.5, 0.22, 0.46), Vector3(17.7, 0.22, 1.4)]:
		_box("Baseboard", pos, Vector3(4.6, 0.16, 0.08), Color(0.035, 0.043, 0.043))
...
func _create_throwables() -> void:
	var positions := [Vector3(1.6, 0.45, 1.4), Vector3(5.2, 0.45, -1.5), Vector3(11.8, 0.45, 1.3), Vector3(14.1, 0.55, 0.8)]
	var models := [
	...
	for i in range(positions.size()):
		...
		item.display_name_key = ["item.laptop", "item.cardboard_box", "item.metal_cog", "item.radio"][i]
...
func _add_path_lights() -> void:
	for pos in [Vector3(0, 2.35, -1.5), Vector3(6.5, 2.25, -1), Vector3(12.0, 2.35, -1.5), Vector3(13, 2.35, -7.8)]:
		_box("CeilingLamp", pos + Vector3(0, 0.30, 0), Vector3(0.82, 0.08, 0.28), Color(0.75, 0.82, 0.70))
		var light := OmniLight3D.new()
		light.position = pos
		light.light_color = Color(0.82, 0.86, 0.72)
...
func _spawn_actors() -> void:
	...
	enemy.position = Vector3(12.8, 0.35, -0.4)
	add_child(enemy)
	var patrol_points: Array[Vector3] = [Vector3(11.2, 0.35, -2.2), Vector3(15.0, 0.35, 1.4), Vector3(14.2, 0.35, -3.0)]
	...
	civilian.position = Vector3(13.4, 0.35, -7.4)
	add_child(civilian)
...
func _build_interaction_overlay(canvas: CanvasLayer) -> void:
	...
	hud_interaction.anchor_left = 0.30
	hud_interaction.anchor_right = 0.70
	hud_interaction.anchor_top = 0.73
	hud_interaction.anchor_bottom = 0.79
	canvas.add_child(hud_interaction)
...
func _world_to_minimap(pos: Vector3) -> Vector2:
	...
	var y := remap(pos.z, 2.5, -10.5, 55.0, 160.0)
	return Vector2(clamp(x, 18.0, 236.0), clamp(y, 45.0, 160.0))
